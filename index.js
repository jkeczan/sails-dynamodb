    ...
    var adapter = {
        ...
        find: function(connection, collectionName, options, cb) {
            ...
            if (options && 'where' in options && _.isObject(options.where)) {
                ...
                query = null;
                if (options.where.id) {
                    options.where.strainGaugeID = options.where.id;
        ...
        join: function(connectionName, collectionName, criteria, cb) {
            ...
            _runJoins({
                ...
                $find: function(collectionIdentity, criteria, cb) {
                    var collection = _collectionReferences[collectionIdentity],
                        model = adapter._getModel(collectionIdentity),
                        query = null,
                        error;
        ...
        _whichIndex: function(collectionName, fields) {
            ...
            //TODO: RANGE KEY COLUMN FIELD IS INCORRECT; IT SHOULD THE RANGE KEY OF THE ASSOCIATION, NOT DEFAULTED TO ID
            ...
            if (primaryHash && primaryRange) {
                //console.log('primary range: ' + primaryRange)
                return {
                    index: 'primary',
                    hash: primaryHash,
                    range: primaryRange
